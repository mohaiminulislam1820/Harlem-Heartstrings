    ...
    return (
        <tr>
            ...
            <td className='pl-6 flex gap-2 py-2'>
                <button className='btn-table bg-yellow-400' disabled={role == 'instructor' ? true : false} onClick={() => handleUpdateRole('instructor')}>Make Instructor</button>
                <button className='btn-table bg-blue-500 text-white' disabled={role == 'admin' ? true : false} onClick={() => handleUpdateRole('admin')}>Make Admin</button>
            </td>
        </tr>
    );
};
